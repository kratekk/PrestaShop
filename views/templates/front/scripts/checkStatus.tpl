...
function DotpayStatusChecker(parent, config) {
    ...
    var checkInt = setInterval(function(){
        ...
        $.get(config.target, {"orderId": config.orderId}, function(e){
            ...
            switch(data.code) {
                case -1://NOT EXIST
                    setErrorMessage(config.messages.notFound);
                ...
                case 2://SUCCESS
                    finish(checkInt);
                    setSuccessMessage(getMessageWithStatus(config.messages.success, data.status));
                    setTimeout(function(){location.href=config.redirect;}, config.delay*1000);
                    break;
                case 3://TOO MANY
                    finish(checkInt);
                    setWarningMessage(getMessageWithStatus(config.messages.tooMany, data.status));
                    break;
                case 4://OTHER STATUS
                    finish(checkInt);
                    setInfoMessage(getBaseMessage(data.status));
                    break;
